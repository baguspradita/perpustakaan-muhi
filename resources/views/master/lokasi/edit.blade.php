<x-app-layout>
    ...
    <div class="mb-8">
        <div class="flex items-center gap-3 mb-2">
            ...
            <h2 class="text-3xl font-extrabold text-slate-800 tracking-tight">Edit Lokasi</h2>
        </div>
        <p class="text-slate-500 font-medium ml-11">Perbarui data lokasi penyimpanan di bawah.</p>
    </div>
        ...
        <form action="{{ route('lokasi.update', $lokasi->id) }}" method="POST">
            ...
            <div class="mb-8">
                ...
                <select name="status" id="status" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition">
                    ...
                    <option value="nonaktif" {{ $lokasi->status === 'nonaktif' ? 'selected' : '' }}>Tidak Aktif</option>
                </select>
                @error('status')
                    <p class="mt-2 text-sm text-red-600">
                        {{ $message }}
