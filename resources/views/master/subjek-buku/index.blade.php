<x-app-layout>
    ...
    <div class="bg-white rounded-3xl border border-slate-100 shadow-md overflow-hidden">
        ...
        <div class="px-8 py-8 border-b border-slate-100 flex items-center justify-between">
            <div>
                <h3 class="text-lg font-bold text-slate-800 leading-snug">Daftar Subjek Buku</h3>
                <p class="text-sm text-slate-500">Total: {{ count($subjek) }} subjek</p>
            </div>
            <a href="{{ route('subjek-buku.create') }}" class="px-6 py-3 bg-indigo-600 text-white font-semibold rounded-xl hover:bg-indigo-700 active:bg-indigo-800 transition-colors flex items-center gap-2 shadow-md">
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100">
                        ...
                        <th class="px-8 py-4 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">Kode DDC</th>
                        <th class="px-8 py-4 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">Nama Subjek</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        <td class="px-8 py-4 text-sm text-slate-600">{{ ($subjek->currentPage() - 1) * $subjek->perPage() + $loop->iteration }}</td>
                        <td class="px-8 py-4 text-sm font-semibold text-indigo-600">{{ $item->kode_ddc }}</td>
                        <td class="px-8 py-4 text-sm font-semibold text-slate-800">{{ $item->nama_subjek }}</td>
                        <td class="px-8 py-4 text-sm text-slate-600">{{ \Illuminate\Support\Str::limit($item->deskripsi, 50, '...') ?? '-' }}</td>
                        ...
                        <td class="px-8 py-4 text-sm flex gap-2">
                            ...
                            <form action="{{ route('subjek-buku.update-status', $item->id) }}" method="POST" class="inline">
                                @csrf
                                @method('PATCH')
                    ...
                    <tr>
                        <td colspan="6" class="px-8 py-12 text-center">
                            ...
                            <p class="text-slate-500 font-medium">Tidak ada data subjek</p>
                            <p class="text-slate-400 text-sm">Mulai dengan menambahkan subjek baru</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
